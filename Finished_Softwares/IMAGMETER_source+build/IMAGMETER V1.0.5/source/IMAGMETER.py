...
#  Create Main window + icon
    ...
    root.protocol('WM_DELETE_WINDOW', w.quitApp) 
    root.mainloop()
...
class GUI(ttk.Frame):
    def __init__(self, parent, name='IMAGEMETER'):
        ...
        self.parent = parent
        self.parent.title("Imagmeter PC DATA V1.0.4")		
        self.fname = ""
        self.fileObj = None
    ...
    def _createCombo(self, parent, Ports):
        ...
        self.Combo.grid(in_ = comboFrame, row = 0, column = 1, sticky = (W, E), ipadx = 10)
		
		# set frame resizing priorities
        comboFrame.rowconfigure(0, weight=1)
        comboFrame.columnconfigure(0, weight=1)
        comboFrame.columnconfigure(1, weight=2)
    ...
    def createFile(self, fname):
        ...
        if fname != None:
            if os.access(os.path.dirname(fname), os.W_OK):				
                ...
                fileObj.write('PROBE 1'+'\t'+'\t'+'\t'+'PROBE 1'+'\t'+'\t'+'\t'+'PROBE 1'+'\t'+'\t'+'\t'+'PROBE 2'+'\t'+'\t'+'\t'+'PROBE 2'+'\t'+'\t'+'\t'+'PROBE 2'+'\t'+'\t'+'\t'+'PROBE 3'+'\t'+'\t'+'\t'+'PROBE 3'+'\t'+'\t'+'\t'+'PROBE 3'+'\n')
                fileObj.write('X DC\t'+'X AC\t'+'X AC+DC\t'+'Y DC\t'+'Y AC\t'+'Y AC+DC\t'+'Z DC\t'+'Z AC\t'+'Z AC+DC\t'+'X DC\t'+'X AC\t'+'X AC+DC\t'+'Y DC\t'+'Y AC\t'+'Y AC+DC\t'+'Z DC\t'+'Z AC\t'+'Z AC+DC\t'+'X DC\t'+'X AC\t'+'X AC+DC\t'+'Y DC\t'+'Y AC\t'+'Y AC+DC\t'+'Z DC\t'+'Z AC\t'+'Z AC+DC\n')
                return fileObj
...
class Acquisition(threading.Thread):
    ...
    def __init__(self, queue, port = 'COM9'):
        ...
        self.datacol = [0, 1, 3, 4, 6, 7, 9, 10, 12, 13, 15, 16, 18, 19, 21, 22, 24, 25, 2, 5, 8, 11, 14, 17, 20, 23, 26]
        self.queue = queue
        self.portCOM = port
        self.portCOM_old = ''
        self.Acq = True
    ...
    def run(self):
        ...
        while self.runCOM :
            ...
            if str(self.portCOM)[0:3]=='COM':
                try:
                    self.ser = Serial(self.portCOM, timeout=None, xonxoff=False, rtscts=False, dsrdtr=False)
