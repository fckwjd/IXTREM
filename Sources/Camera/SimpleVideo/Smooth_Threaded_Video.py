...
class App:
    ...
    def process_frame(self, t0, old_frame):
        ret, frame = self.cap.read()
        # frame = self.equalize_histogram(frame)
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        frame = cv2.addWeighted(frame, 0.05, old_frame, 0.95, 0)
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self.pending) < self.threadn:
                ...
                last_frame_time = t
                self.task = self.pool.apply_async(self.process_frame, (t, self.old_frame))
