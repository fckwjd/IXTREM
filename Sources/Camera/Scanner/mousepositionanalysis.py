...
class MousePosAnalysis:
    ...
    def scalezVar(self):
        ...
        self.idx = 0
        for self.idx in range(self.ch_nb):
            self.zMin = np.min(self.zVar_m[self.idx])
            self.zMax = np.max(self.zVar_m[self.idx])
            # Set the sensor data array around 0 in order to sort it
            self.zVar = np.hstack((self.zVar, self.zVar_m[self.idx] - self.zMean[self.idx]))
            self.xPos = np.hstack((self.xPos, self.xPos_m[self.idx]))
            self.yPos = np.hstack((self.yPos, self.yPos_m[self.idx]))
    ...
    def normalizeCmap(self):
        ...
        self.zMin = np.min(self.zVar_s)
        self.zMax = np.max(self.zVar_s)
    ...
    def saveData(self, fname):
        dataT = np.transpose(np.array((self.xPos_s, self.yPos_s, self.zVar_s)))
