...
class Carto3D:
    ...
    def startAnfatec(self):
        # Start Anfatec Task
        self.task = an.Anfatec(self.queue, setFrequency=10000.0, setAmplitude=2.0, setPhase=0.0, setHarm=1,
                               setPllOn=0, setTimeConst=12, setRollOff=0, setHardGain=10, setCoupling=0,
                               setTimeBase=0.001, setAux1=0, setAux2=1, setChAux1=10, setChAux2=11,
                               setAmplitudeAux1=1.0, setAmplitudeAux2=1.0, wait=0.1, keep=False)
        try:
            self.timeBase = task.getTimeBase()
        ...
        while self.camera.getIsRunning() and self.task.getIsRunning():
            try:
                ...
                for self.idx in range(self.ch_nb):
                    self.xPosT.append(
                        self.xPos * self.scale + (self.idx - (self.ch_nb + 1) / 2) * self.xOffset * np.sin(-self.angle))
                    self.yPosT.append(
                        self.yPos * self.scale + (self.idx - (self.ch_nb + 1) / 2) * self.xOffset * np.cos(-self.angle))
                    self.zVar_t = np.mean(self.zVar[self.data_nb * (self.idx):self.data_nb * (self.idx + 1) - 1])
    ...
    def calibZData(self):
        ...
        for self.idx_ch in range(self.ch_nb):
            self.zMean.insert(self.idx_ch, np.mean(self.zMean_t[self.idx_ch]))
