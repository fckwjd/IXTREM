...
class GUI:
    ...
    def start_btn(self):
        ...
        if self.zMean != []:
            self.carto = carto3Dcam.Carto3D(camcorr=self.camcorr, dev_name=self.dev_name, ch_name=self.ch_name,
                                            ...
                                            color_sensor=self.color_sensor, color_dir=self.color_dir)

            self.mouse = mpa.MousePosAnalysis(zMean=self.zMean)
            ...
            self.thread = Acquisition(self.camcorr, self.dev_name, self.ch_name, self.data_nb, self.rate,
                                      ...
                                      self.subInterp, self.queue, self.queuestart, self.carto, self.mouse,
                                      self.isCalibrating)
            self.thread.start()
    ...
    def calibSensor(self):
        ...
        self.carto = carto3Dcam.Carto3D(dev_name=self.dev_name, ch_name=self.ch_name, data_nb=self.data_nb,
                                        rate=self.rate)
        ...
        self.thread.start()
        self.processAcquisition()
        self.zMean = self.carto.getZMean()
    ...
    def onpress(self, event):
        ...
        Z, Xline, Yline = self.mouse.getDataAtPoint(event.xdata, event.ydata)
        self.mouse.plotIntersec(Xline, Yline, self.subXFigCut, self.subYFigCut)
        self.showCutFigure()
        self.canvas_figCut.show()
...
class Acquisition(Thread):
    ...
    def run(self):
        ...
        if self.isCalibrating:
            ...
            self.mouse.setData(self.xPos, self.yPos, self.zVar)
            self.mouse.setChNb(self.ch_nb)
            if self.zVar != []:
