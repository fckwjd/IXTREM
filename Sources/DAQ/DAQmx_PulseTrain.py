...
class ContinuousPulseTrainGeneration():
    ...
    def __init__(self, period=0.01, duty_cycle=0.5, counter="/Dev1/ctr0", reset=False):
        if reset:
            DAQmxResetDevice(counter.split('/')[0])
        taskHandle = TaskHandle(0)
        DAQmxCreateTask("", byref(taskHandle))
        DAQmxCreateCOPulseChanFreq(taskHandle, counter, "", DAQmx_Val_Hz, DAQmx_Val_High,
                                   1.0, 1 / float(period), duty_cycle)
...
if __name__ == "__main__":
    pulse_gene1 = ContinuousPulseTrainGeneration(0.01, 0.5, "Dev1/ctr0", reset=True)
    pulse_gene1.start()
