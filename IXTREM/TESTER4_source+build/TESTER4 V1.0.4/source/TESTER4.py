...
class GUI(ttk.Frame):
    def __init__(self, parent, name='TESTER 4'):
        ...
        self.parent = parent
        self.parent.title("TESTER 4 PC DATA V1.0.4")
        self.fname = ""
        self.fileObj = None
        self.data = []
        self.dataHold = []
    ...
    def run(self):
        ...
        while self.runCOM:
            ...
            if str(self.portCOM)[0:3] == 'COM':
                try:
                    self.ser = Serial(self.portCOM, baudrate=9600, timeout=500, xonxoff=False, rtscts=False,
                                      dsrdtr=False)
                        ...
                        if len(self.data) == 8:
                            if self.data[0] != '5':
                                ...
                                if self.data[6:8] == 'Lu' or self.data[6:8] == 'UW':
                                    ...
                                    self.dataList = [self.selectMode(self.data[0]), self.data[1:6],
                                ...
                                if self.dataListFilled:
                                    if self.data[6:8] == 'Lu' or self.data[6:8] == 'UW':
                                        self.dataList = self.dataList + [self.selectMode(self.data[0]), self.data[1:6],
                                                                         self.selectUnits(self.data[6:8])]
                                    else:
                                    ...
                                    if self.data[6:8] == 'Lu' or self.data[6:8] == 'UW':
                                        ...
                                        self.dataList = [self.selectMode(self.data[0]),
                                                         self.comaInValue(self.data[1:6]),
                                                         self.selectUnits(self.data[6:8])]
                            ...
                            if len(self.dataHold) == 0:
                                if self.data[7:9] == 'Lu' or self.data[7:9] == 'Uw':
                                    self.dataHold = [self.data[0], self.selectMode(self.data[1]), self.data[2:7],
                                                     self.selectUnits(self.data[7:9])]
                                else:
                                    self.dataHold = [self.data[0], self.selectMode(self.data[1]),
                                                     self.comaInValue(self.data[2:7]), self.selectUnits(self.data[7:9])]
                            else:
                                if self.data[0] == str(self.temploop - 1):
                                    if self.data[7:9] == 'Lu' or self.data[7:9] == 'Uw':
                                        self.dataHold = self.dataHold + [self.data[0], self.selectMode(self.data[1]),
                                                                         self.data[2:7],
                                                                         self.selectUnits(self.data[7:9])]
                                    else:
                                        self.dataHold = self.dataHold + [self.data[0], self.selectMode(self.data[1]),
                                                                         self.comaInValue(self.data[2:7]),
                                                                         self.selectUnits(self.data[7:9])]
                            if self.temploop == 10:
